...
# Installation
  ...
  2. Install the dependencies:
     ...
     ```bash
      pip install -e .
     ```
Optional: if you encounter issues with `pycddlib`, follow the troubleshooting instructions provided in the `pycddlib` documentation, or the ones provided in SimplexEmbeddingGPT.git documentation.
...
In order to reproduce the computations in the paper, the files vendored from SimplexEmbeddingGPT.git were modified accordingly:
  - In the file `mathtools.py`, the line `import cdd` was replaced with:
     ```python
     import cdd.gmp as cdd
     ```
  - In the file `mathtools.py`, the function `FindStateConeFacets` was modified to the following:
     ```python
    def FindStateConeFacets(S):
        ...
        return H_S
     ```
  - In the file `mathtools.py`, the function `FindEffectConeFacets` was modified to the following:
     ```python
   ...
   - In the file `simplexEmbedding.py`, the function `SimplicialConeEmbedding` was modified to the following:
     ```python
     ...
     ```
     This ensures that the main solver for the linear program will be ECOS, with an alternative try with CLARABEL. If none of the solvers manages to resolve the program, it will return a robustness of contextuality `r = 2`, which will later be ignored when computing typicality frequencies. Finally, the function SimplexEmbedding in this file no longer computes the ontological model for the depolarised GPT, outputting exclusively the robustness values. This enhances efficience, since we are not interested on the explicit noncontextual implementation of the depolarised scenarios in this work.
